...
- 특정 코드를 함수로 분리할 때 꼭 재사용이 필요할 때만 하는게 아닌 특정 기능의 이름을 붙혀주기 위해서도 함수로 분리한다.

- 상태관리 - 클라이언트에서 상태관리는 왜필요한가..? 서버와 클라이언트에서 상태관리를 하면서 왜 상태를
  이중으로 관리해야 하는지 의문이 들었다. 어차피 서버에서 데이터를 관리하고 있기 때문에 필요할 때마다 요청해서
  사용하면 될거 같은데 상태관리의 중요성, 상태관리 라이브러리 등에 개념들은 넘치는 걸까?

- 상태관리가 생긴 배경은 프레임워크와 컴포넌트로 개발하는 과정이 보편화 되다보니 컴포넌트 단위로 데이터를 관리 하는데 컴포넌트 구조가 복잡해지면 props를 전달하기 위해 계속 props를 연결해야하는 문제가 발생하기 때문에 데이터를 컴포넌트가 아니라 공통의 곳에서 관리를 해야겠다는 생각에서 만들어 지게 되었다고 한다.

- React를 아직 학습하진 않았지만 React에서도 비슷하게 서버와 클라이언트에서 상태관리를 하고 있다.
  그래서 React-Query가 요즘 유행하고 대안으로 떠오른다고 한다. 간단하게 읽어 봤지만  
  Server와 Client 사이 비동기 로직들을 손쉽게 다루게 해주는 도구로 서로 뒤엉키지 않고 각각
  관리하게 해주는 라이브러리라고 한다.

- 아직 선행 개념들이 안잡혀있어서 글을 읽어도 이해가 잘되지 않는다. 클라이언트 store없이 코드를 작성했을 때 불편했던 점은 특정 데이터를 불러오는 코드를 작성 할때마다 서버에 데이터를 요청해줘야 하기 때문에 효율적인 측면과 코드를 이중으로 작성하는 불편함이 있었다.
  React 공부를 할때 다시한번 찾아 봐야겠다.
